...
//     Source_url: string;
// }
export const getHeadlineQuotes = async () => {
    try {
...
export const getHeadline = async (url) => {
    try {
        ...
        // const button = ''
        // Wait for the button to be available and click it
        await page.waitForSelector('#carousel_light > div > div.carousel__items > div.carousel__item.carousel__item-0 > div.carousel__item__title > h2 > a', { timeout: 5000 });
        // await page.waitForTimeout(3000);
        await page.click('#carousel_light > div > div.carousel__items > div.carousel__item.carousel__item-0 > div.carousel__item__title > h2 > a');
        const photoSelector = '#sports_header_image > img';
        ...
        const quotes = await page.evaluate(() => {
            const title = document.querySelector("#sports_article_content > div.padding > div.article__title > h1")?.innerText || "No title found";
            const body = document.querySelector("#sports_article_writeup > p:nth-child(1)")?.innerText || "No context found";
            // const source_url = saved // Replace with the actual URL if needed
            return { title, body };
        });
        quotes.source_url = savedPhoto; // Add the image URL to the quotes object
